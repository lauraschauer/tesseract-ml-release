...
def assemble_arrays():
    ...
    np.save(os.path.join(DATA_DIR, 'dexray_base/X_train.npy'), X_train)
    np.save(os.path.join(DATA_DIR, 'dexray_base/X_test.npy'), X_test)
    np.save(os.path.join(DATA_DIR, 'dexray_base/y_train.npy'), y_train)
    np.save(os.path.join(DATA_DIR, 'dexray_base/y_test.npy'), y_test)
...
def main():
    ...
    model.save("/scratch/users/mbenali/tesseract-ml-release/model-50k-base")
